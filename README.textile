...
p. Getting things up and running is easy once you have gotten "Composer installed":http://getcomposer.org/doc/00-intro.md, after which, you can freely use Composer to install and update any plugins and themes that are offering Composer packages. Each theme and plugin is just like any other Composer package, and you mange them all the same. Same instructions that are in Composer's "documentation":http://getcomposer.org/doc/ apply for Textpattern plugins and themes.

p. After you have Composer configured and its requirements installed, using the tool is pretty easy. Any themes and plugins you are add to the Textpattern installation that is found in the current working directory you are in. In other words to the installation that is in the same directory as Composer's @composer.json@ file. To get started navigate to the directory you have installed Textpattern:

bc. $ cd ~/domains/example.test/httpdocs

p. Once there, you can start consuming theme and plugin packages with the normal composer commands such as @update@, @install@ and @require@. You should start by locking in your installed Textpattern version.

bc. $ composer.phar require "textpattern/lock 4.5.4"

p. Where @4.5.4@ is the Textpattern version you have. This makes sure the packages you install are compatible with the Textpattern version you have. Now you are free to go and get plugins and themes. For instance installing "rah_replace":http://rahforum.biz/plugins/rah_replace doesn't take more than a command.
...
* Fixed: Plugin installer. Now correctly uninstalls plugin and then deletes files, instead of trying to do it the other way around.
