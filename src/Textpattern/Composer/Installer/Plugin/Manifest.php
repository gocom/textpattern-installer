...
class Manifest extends Base
{
    ...
    protected function code()
    {
        ...
        if (!empty($this->manifest->autoload) && is_dir($this->dir . '/src'))
        {
     ...
     *
     * The whole project file structure is encoded to the plugin source.
     */

    protected function autoload()
    {
        $files = new RecursiveDirectoryIterator($this->dir . '/src');
        $files = new RecursiveIteratorIterator($files, RecursiveIteratorIterator::SELF_FIRST);
        $out = array();

        foreach ($files as $file)
        {
            if ($file->isFile() && $file->isReadable() && pathinfo($file->getFilename(), PATHINFO_EXTENSION) === 'php' && $contents = file_get_contents($file->getPathname()))
            {
                $class = str_replace('/', '\\', substr($file->getPathname(), strlen($this->dir) + 4, -4));
                $out[$class] = trim(preg_replace('/^<\?(php)?|\?>$/', '', $contents), "\r\n");
            }
        }

        $json = addSlashes(json_encode($out));
        $md5 = md5($this->manifest->name) . '_' . md5(uniqid('', true));

        return <<<EOF

/**
 * Autoloader generated by plugin installer.
 */

class Rah_Textpattern_Composer_Installer_Autoloader_{$md5}()
{
    /**
     * Filesystem structure as a JSON representation.
     *
     * @var string
     */

    private \$filesystem = '{$json}';

    /**
     * Constructor.
     *
     * Registers the autoloader when the class is initialized.
     */
    ...
    /**
     * Basic PSR-0 compatible autoloader.
     */

    public functon autoload(\$class)
    {
        \$class = str_replace('_', '\\\\', \$class);

        if (isset(\$this->filesystem[\$class]))
        {
            eval(\$this->filesystem[\$class]);
            return true;
        }
    }
}

new Rah_Textpattern_Composer_Installer_Autoloader_{$md5}();
